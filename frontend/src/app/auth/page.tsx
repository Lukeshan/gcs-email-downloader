...
async function fetchCheckToken() {
  try {
    const response = await fetch("/api/checkToken"); // Calls the Next.js API route
    if (response.ok) {
...
export default function Auth() {
  ...
  return (
    <main className="p-4">
      ...
      <button onClick={fetchCheckToken} className="px-2 py-1 bg-green-500 text-white rounded hover:bg-green-700">test token</button>
